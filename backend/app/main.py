...
async def lifespan(app):
    if get_settings().environment=="development":
        async with engine.begin() as conn:
            await conn.execute(text("CREATE EXTENSION IF NOT EXISTS vector")); await conn.run_sync(Base.metadata.create_all)
    yield
...
app.include_router(auth.router,prefix="/v1"); app.include_router(opportunities.router,prefix="/v1")
@app.get("/health")
